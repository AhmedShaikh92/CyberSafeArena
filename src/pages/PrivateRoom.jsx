...
function CodeChar({ char, index }) {
  return (
    <motion.span
      ...
      className="inline-block w-10 h-12 flex items-center justify-center font-display text-2xl font-black rounded-sm"
      style={{
        color: '#00d4ff',
...
function MemberSlot({ member, isHost, onKick, currentUserId, currentUserRole }) {
  // For the current user, always use the live role from the auth store
  // instead of the potentially stale role in the socket payload
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div>
          <div className="flex items-center gap-2">
            <span className="text-sm font-mono text-white font-bold">{member.username}</span>
            {isMe && <span className="text-xs font-mono text-arena-muted">(you)</span>}
            {member.isHost && (
              <span className="text-xs font-display px-1.5 py-0.5 tracking-wider"
      ...
      <div className="flex items-center gap-2">
        {/* Always render badge using effectiveRole */}
        {effectiveRole && (
        ...
        {isHost && !isMe && (
          <motion.button
            ...
            whileHover={{ background: '#ff224415', scale: 1.05 }}
          >
            KICK
          </motion.button>
        )}
...
function EmptySlot({ index }) {
  return (
    <motion.div
      ...
      style={{ border: '1px dashed rgba(30,41,59,0.8)', background: 'transparent' }}
    >
      <div className="w-9 h-9 rounded-sm border border-dashed border-arena-border flex items-center justify-center">
      ...
      <div>
        ...
        <div className="flex gap-0.5 mt-1">
          {[0, 1, 2].map((i) => (
            <motion.div
              key={i}
              className="w-1 h-1 rounded-full bg-arena-muted"
...
const SCENARIOS = [
  ...
  { value: 'corporate_breach', label: 'Corporate Breach' },
  { value: 'ransomware_response', label: 'Ransomware Response' },
  { value: 'apt_infiltration', label: 'APT Infiltration' },
...
export default function PrivateRoom() {
  ...
  // Handle match completion with XP rewards
  const handleMatchComplete = (performanceScore, result) => {
    const xpGain = applyMatchXP(performanceScore, result)
    if (result === 'win') {
      incrementWin()
    } else {
      incrementLoss()
  ...
  // Resolve current user ID — backends use _id (Mongo) or id
  const currentUserId = user?._id || user?.id
  // Always read role live from the auth store, never from the socket payload
        ...
        <AnimatePresence mode="wait">
          ...
          {view === 'choose' && (
            ...
            >
              <div className="text-center mb-8">
                ...
                <h1 className="font-display text-3xl font-black tracking-widest">
                  <span className="neon-text-blue">PRIVATE</span>
                  <span className="neon-text-red ml-2">ROOM</span>
                </h1>
                <p className="text-sm font-mono text-arena-muted mt-2">
                  Challenge a specific operator directly
                </p>
              </div>
              ...
              <div className="space-y-3">
                <motion.button
                  ...
                  className="w-full p-5 rounded-sm text-left relative overflow-hidden group"
                  style={{ background: 'rgba(0,212,255,0.04)', border: '1px solid rgba(0,212,255,0.25)' }}
                  whileHover={{ background: 'rgba(0,212,255,0.08)', boxShadow: '0 0 25px rgba(0,212,255,0.15)' }}
                >
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-sm flex items-center justify-center text-2xl shrink-0"
                      style={{ background: 'rgba(0,212,255,0.1)', border: '1px solid rgba(0,212,255,0.3)' }}>
                      ⚡
                    </div>
                    <div>
                      <p className="font-display text-base font-bold neon-text-blue tracking-wider">CREATE ROOM</p>
                      <p className="text-xs font-mono text-arena-muted mt-0.5">Generate a code and share with your squad</p>
                    </div>
                    <span className="ml-auto neon-text-blue text-lg">→</span>
                  </div>
                </motion.button>

                <motion.button
                  onClick={() => setView('join')}
                  className="w-full p-5 rounded-sm text-left relative overflow-hidden"
                  style={{ background: 'rgba(255,34,68,0.04)', border: '1px solid rgba(255,34,68,0.25)' }}
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="font-display text-base font-bold neon-text-red tracking-wider">JOIN ROOM</p>
                      <p className="text-xs font-mono text-arena-muted mt-0.5">Enter a code from another operator</p>
                    </div>
                    <span className="ml-auto neon-text-red text-lg">→</span>
                  </div>
                </motion.button>

                <motion.button
                  onClick={() => navigate('/dashboard')}
                  className="w-full text-center py-2 text-xs font-mono text-arena-muted"
          ...
          {view === 'create' && (
            <motion.div
              ...
              animate={{ opacity: 1, x: 0 }}
              exit={{ opacity: 0, x: -30 }}
              className="glass-panel rounded-sm p-7"
            >
              <button onClick={() => setView('choose')} className="text-xs font-mono text-arena-muted mb-5 flex items-center gap-1 hover:text-slate-300">
                ← Back
              </button>
              <h2 className="font-display text-xl font-bold neon-text-blue tracking-widest mb-5">CONFIGURE ROOM</h2>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {DIFFICULTIES.map((d) => {
                      const colors = { easy: '#10b981', medium: '#f59e0b', hard: '#ff6600', elite: '#ff2244' }
                      ...
                      return (
                        <motion.button
                          ...
                          style={{
                            color: colors[d],
                            border: `1px solid ${selected ? colors[d] : colors[d] + '30'}`,
                            background: selected ? `${colors[d]}15` : 'transparent',
                            textShadow: selected ? `0 0 8px ${colors[d]}` : 'none',
                          }}
                          whileHover={{ background: `${colors[d]}10` }}
                        >
                          {d}
                        </motion.button>
                      )
          ...
          {view === 'join' && (
            <motion.div
              ...
              className="glass-panel rounded-sm p-7"
            >
              <button onClick={() => { setView('choose'); setJoinError('') }} className="text-xs font-mono text-arena-muted mb-5 flex items-center gap-1 hover:text-slate-300">
                ← Back
              </button>
              <h2 className="font-display text-xl font-bold neon-text-red tracking-widest mb-2">ENTER ROOM CODE</h2>
              ...
              <motion.button
                ...
                className="mt-6 w-full rounded-sm px-6 py-3 font-display text-sm font-bold tracking-widest uppercase transition-all"
                style={{
      ...
      {Array.from({ length: LENGTH }).map((_, i) => {
        ...
        return (
          <motion.input
            ...
            onPaste={handlePaste}
            onChange={() => {}}
            className="w-12 h-14 text-center font-display text-2xl font-black rounded-sm outline-none caret-transparent select-none"
            style={{
              ...
              boxShadow: isFilled ? '0 0 12px rgba(255,34,68,0.15)' : 'none',
            }}
...
function LobbyView({ roomCode, members, isHost, config, currentUserId, currentUserRole, canStart, onKick, onUpdateConfig, onStart, onLeave, status }) {
  return (
        ...
        >
          <div className="px-6 py-4 border-b border-arena-border flex items-center justify-between"
            ...
            <div>
              ...
              <h2 className="font-display text-lg font-bold neon-text-blue tracking-wider">OPERATOR LOBBY</h2>
            </div>
            <div className="flex items-center gap-2">
          ...
          <div className="p-6 space-y-6">
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-display text-xs tracking-widest text-arena-muted">
                  OPERATORS ({members.length}/{config.maxPlayers || 8})
                </h3>
                <span className="text-xs font-mono">
                  {members.length >= 2
                    ? <span style={{ color: '#10b981' }}>✓ Ready to launch</span>
                    : <span style={{ color: '#f59e0b' }}>Waiting for players...</span>
                  }
                </span>
              </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                {
                  label: 'SCENARIO',
                  value: SCENARIOS.find(s => s.value === config.scenario)?.label || 'Random',
                  color: '#00d4ff',
                },
                {
                  label: 'DIFFICULTY',
                  value: (config.difficulty || 'medium').toUpperCase(),
                  color: { easy: '#10b981', medium: '#f59e0b', hard: '#ff6600', elite: '#ff2244' }[config.difficulty] || '#f59e0b',
                },
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full bg-black/40 border border-arena-border rounded-sm px-2 py-1.5 text-xs font-mono text-white outline-none"
                    >
                      {SCENARIOS.map(({ value, label }) => (
                        <option key={value} value={value} style={{ background: '#0f172a' }}>{label}</option>
                      ))}
            ...
            <div className="flex gap-3">
              <motion.button
                onClick={onLeave}
                className="btn-danger flex-1 rounded-sm"
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
              >
              ...
              {isHost && (
                <motion.button
                  ...
                  className="flex-1 rounded-sm px-6 py-3 font-display text-sm font-bold tracking-widest uppercase transition-all"
                  style={canStart ? {
                  ...
                  whileHover={canStart ? { boxShadow: '0 0 35px rgba(0,212,255,0.4)' } : {}}
                  animate={canStart ? {
              ...
              {!isHost && (
                <div className="flex-1 flex items-center justify-center py-3 rounded-sm text-xs font-mono text-arena-muted"
